  ...
  <div class="inbound-list">
    ...
    <SearchForm :model="searchForm" @search="handleSearch" @reset="handleReset">
      <el-form-item label="单号">
        <el-input v-model="searchForm.keyword" placeholder="请输入入库单号" clearable />
      </el-form-item>
    ...
    <el-table :data="tableData" v-loading="loading" border>
      ...
      <el-table-column prop="warehouseName" label="入库仓库" width="100" />
...
import { listInbound, getInbound, auditInbound, executeInbound, cancelInbound } from '@/api/inbound'
...
const handleSearch = async () => {
  loading.value = true
  try {
    const res: any = await listInbound({
...
const handleAudit = (row: any) => {
  ElMessageBox.confirm('确定要审核通过该入库单吗？', '提示', { type: 'warning' }).then(async () => {
...
const handleExecute = (row: any) => {
  ElMessageBox.confirm('确定要执行该入库单吗？执行后将更新库存。', '提示', { type: 'warning' }).then(async () => {
    await executeInbound(row.id)
